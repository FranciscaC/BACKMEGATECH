...
public class StudentModelAssembler implements RepresentationModelAssembler<StudentDTO, EntityModel<StudentDTO>> {
    ...
    @Override
    public @NonNull EntityModel<StudentDTO> toModel(@NonNull StudentDTO studentDTO) {
        EntityModel<StudentDTO> studentDTOEntityModel = EntityModel.of(
                ...
                linkTo(methodOn(StudentController.class).findAllStudyingStudents()).withRel("studying-students"),
                linkTo(methodOn(StudentController.class).findAllGraduatedStudents()).withRel("graduated-students")
        );
        ...
        if (studentDTO.getAcademicStatusEnum() == AcademicStatusEnum.STUDYING) {
            studentDTOEntityModel.add(linkTo(methodOn(StudentController.class)
                    .updateAcademicStatusToGraduated(studentDTO.getId())).withRel("graduate-student"));
        }
